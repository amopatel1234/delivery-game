...
`ISSUER_ID` and `KEY_ID` are set in `merged.yml`. The App Store provisioning profile name in Fastlane is `Delivery Game App Store` for bundle ID `amishpatel.delivery-game` — create that profile in the Apple portal before the first deploy.
